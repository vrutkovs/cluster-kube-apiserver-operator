...
type observeAPIServerConfigFunc func(ctx context.Context, apiServer *configv1.APIServer, recorder events.Recorder, previouslyObservedConfig map[string]interface{}, listers *configobservation.Listers) (map[string]interface{}, syncActionRules, []error)
...
func observeUserClientCABundle(ctx context.Context, apiServer *configv1.APIServer, recorder events.Recorder, previouslyObservedConfig map[string]interface{}, listers *configobservation.Listers) (map[string]interface{}, syncActionRules, []error) {
...
// specific hostnames.
func observeNamedCertificates(ctx context.Context, apiServer *configv1.APIServer, recorder events.Recorder, previouslyObservedConfig map[string]interface{}, listers *configobservation.Listers) (map[string]interface{}, syncActionRules, []error) {
	var errs []error
	...
	for index, namedCertificate := range namedCertificates {
		...
		if _, err := listers.ConfigSecretLister().Secrets(operatorclient.GlobalUserSpecifiedConfigNamespace).Get(ctx, namedCertificate.ServingCertificate.Name); err != nil {
			return previouslyObservedConfig, nil, append(errs, err)
		}
...
func (o *apiServerObserver) observe(ctx context.Context, genericListers configobserver.Listers, recorder events.Recorder, existingConfig map[string]interface{}) (map[string]interface{}, []error) {
	...
	observedConfig, observedResources, errs := o.observerFunc(ctx, apiServer, recorder, previouslyObservedConfig, &listers)
