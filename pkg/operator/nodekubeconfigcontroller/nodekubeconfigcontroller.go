...
func (c NodeKubeconfigController) sync(ctx context.Context, syncContext factory.SyncContext) error {
	...
	ctx, span := tracer.Start(ctx, "ckao.NodeKubeconfigController", trace.WithAttributes(
...
func ensureNodeKubeconfigs(ctx context.Context, client coreclientv1.CoreV1Interface, secretLister corev1listers.SecretLister, configmapLister corev1listers.ConfigMapLister, infrastructureLister configv1listers.InfrastructureLister, recorder events.Recorder) error {
	...
	systemAdminCredsSecret, err := secretLister.Secrets(operatorclient.OperatorNamespace).Get(ctx, nodeSystemAdminClient)
	if err != nil {
		return err
	}
	...
	servingCABundleCM, err := configmapLister.ConfigMaps(operatorclient.TargetNamespace).Get(ctx, kubeApiserverServerCA)
	if err != nil {
		return err
	}
	...
	infrastructure, err := infrastructureLister.Get(ctx, "cluster")
	if err != nil {
		return err
	}
	...
	actualSecret, err := secretLister.Secrets(requiredSecret.Namespace).Get(ctx, requiredSecret.Name)
